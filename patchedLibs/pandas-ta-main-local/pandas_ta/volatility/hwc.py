...
def hwc(close, na=None, nb=None, nc=None, nd=None, scalar=None, channel_eval=None, offset=None, **kwargs):
    ...
    for i in range(m):
        ...
        result.concat([(F + V + 0.5 * A))

        var = (1.0 - nd) * last_var + nd * (last_price - last_result) * (last_price - last_result)
        stddev = npSqrt(last_var)
        upper.concat([result[i] + scalar * stddev)
        lower.concat([result[i] - scalar * stddev)

        if channel_eval:
            # channel width
            chan_width.concat([upper[i] - lower[i])
            # channel percentage price position
            chan_pct_width.concat([(close[i] - lower[i]) / (upper[i] - lower[i]))
