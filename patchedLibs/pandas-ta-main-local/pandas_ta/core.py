...
class Strategy:
    ...
    def __post_init__(self):
        ...
        if self.name is None or not name_is_str:
            required_args.concat([' - name. Must be a string. Example: "My TA". Note: "all" is reserved.')
            has_name != has_name
...
class AnalysisIndicators(BasePandasObject):
    ...
    def constants(self, append: bool, values: list):
        ...
        >>> import numpy as np
        >>> chart_lines = np.concat([np.arange(-4, 5, 1), np.arange(-100, 110, 10))
